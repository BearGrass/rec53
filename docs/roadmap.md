...
### 当前推进主线 — v1.3.x 资源保护实现顺序调整

**目标**：在 `v1.3.0` 已完成主线拆分之后，把后续资源保护工作按“已落地能力 / 下一步优先讨论项 / 后续依赖项”重新排顺。当前判断是：`v1.3.1` 已完成单客户端昂贵请求保护；下一步优先讨论 `zone / 热点域名保护`，再继续更通用的上游并发与全局保险丝问题。
...
- `v1.3.0` 的职责已经完成，当前重点转为调整 `v1.3.1+` 的讨论和实现顺序
- `v1.3.1` 已经把“单客户端昂贵请求并发保护”走通，后续不再占用“下一步先讨论什么”的位置
- 当前更值得先讨论的是 `zone / 热点域名保护`：它更直接面向热点域名、随机子域名和局部流量倾斜问题，也更适合作为 per-client 保护之后的下一层补充边界
...
- [x] 启动 `v1.3.2` 的首次 proposal：单热点 zone 的昂贵路径入口保护
- [ ] 继续细化 `v1.3.2`：zone / 热点域名保护的 spec、task 与边界
- [ ] 再讨论 `v1.3.3`：上游查询并发保护
- [ ] 后续讨论 `v1.3.4`：全局请求并发保险丝
...
当前推荐顺序已经从“先上游并发、再 zone 保护”调整为“先 zone / 热点域名保护、再上游并发保护”。原因是 zone 维度更直接面向热点域名、随机子域名和局部放大问题，也更适合作为 per-client 保护之后的下一层补充边界。
...
**状态**：进行中。首版代码已落地 coarse business-root 选择、observe/protect 单热点入口保护、`REFUSED` 拒绝、聚合指标与基础文档，同步仍可继续补压测与后续收口。
