...
| v1.3.0 | 2026-03 | done | 资源保护版本规划（v1.3.1-v1.3.9） |
| v1.3.1 | 2026-03 | done | 单 IP / per-client 昂贵请求保护与公平性 |
| v1.3.2 | 2026-03 | done | 单热点 zone / 热点域名入口保护 |
...
### 当前推进主线 — v1.3.3 / v1.3.4 上游与全局保护

**目标**：在 `v1.3.1` 和 `v1.3.2` 已完成之后，把后续资源保护工作收敛到 `v1.3.3` 的上游查询并发保护和 `v1.3.4` 的全局请求并发保险丝。`v1.3.3` 的重点是限制递归 / 转发阶段的上游出包压力，同时顺带降低本机 CPU、goroutine 和尾延迟压力；`v1.3.4` 再作为更粗粒度的最后一道系统级闸门。
...
- `v1.3.0` 的职责已经完成，当前重点转为调整 `v1.3.3+` 的讨论和实现顺序
- `v1.3.1` 已经把“单客户端昂贵请求并发保护”走通
- `v1.3.2` 已经把“单热点 zone / 热点域名入口保护”走通，后续不再占用“下一步先讨论什么”的位置
- 当前更值得先讨论的是 `v1.3.3` 上游查询并发保护：它更直接面向上游出包压力，也会连带保护本机 CPU，因此更适合作为热点 zone 之后的下一层补充边界

**下一步焦点**

- [x] 收口 `v1.2.0`：完成 readiness probe、bounded `phase`、生命周期测试与 operator 文档对齐
- [x] 产出 `v1.3.x` 的首版拆分框架
- [x] 完成 `v1.3.1`：单 IP / per-client 昂贵请求保护与开发期验证
- [x] 完成 `v1.3.2`：单热点 zone / 热点域名保护与 OpenSpec 同步
- [ ] 起草 `v1.3.3`：上游查询并发保护的首次 proposal
- [ ] 起草 `v1.3.4`：全局请求并发保险丝的边界与语义
...
**状态**：已完成。`v1.3.0` 已经把资源保护主线拆成可继续讨论的子版本序列；当前工作不再是继续做规划本身，而是调整后续子版本的优先级并逐项推进实现。

**目标**：在运行状态契约稳定之后，把资源保护作为一个需要拆解的大颗粒主题来规划。`v1.3.0` 只负责把后续工作拆成 `v1.3.1-v1.3.9` 的版本序列，明确每一项主要回答什么问题、依赖什么前提、哪些内容暂不纳入。

**任务**

- [x] 产出 `v1.3.1-v1.3.9` 的主题列表和初步命名
- [x] 为每个子版本定义要回答的核心问题与不做项
- [x] 标出实现类版本与设计/观测/测试类版本的区别
- [x] 梳理子版本之间的前置依赖与推荐顺序
- [x] 明确跨版本共享议题：超限语义、warmup 交互、forwarding/iterative 边界、operator 口径
- [x] 更新 roadmap，使 `v1.3` 从单条目变成可展开讨论的版本序列
...
### v1.3.1-v1.3.8 — 资源保护子版本顺序

`v1.3.0` 的交付目标是把 `v1.3` 主线拆成一组可单独讨论、单独实现、单独验证的子版本。当前粗粒度框架已执行到 `v1.3.2`，后续从 `v1.3.3` 开始继续推进：

- `v1.3.1` 单 IP / per-client 限制与公平性（done）
- `v1.3.2` 单热点 zone / 热点域名保护（done）
- `v1.3.3` 上游查询并发保护：控制递归 / 转发阶段的上游出包压力（进行中，首版共享静态闸门、软降级、指标与测试已落地）
- `v1.3.4` 全局请求并发保险丝：控制系统级总压力
- `v1.3.5` 后台任务资源边界
- `v1.3.6` 超限返回语义
- `v1.3.7` 观测指标与 operator 策略
- `v1.3.8` 测试矩阵、压测方法与回滚策略

当前推荐顺序已经从“先 zone / 热点域名保护、再上游并发保护”推进到“zone 已完成，接下来优先讨论上游并发保护，再继续全局请求并发保险丝”。原因是上游出包压力既会拖累上游服务器，也会反过来消耗本机 CPU 与等待资源，因此比单纯的全局保险丝更适合作为下一层边界。

### v1.3.1 — 单 IP / per-client 限制与公平性

**状态**：已完成实现与开发期验证。后续若需要 release 口径、文档归档或 change archive，再单独收口。
...
**状态**：已完成。首版代码已落地 coarse business-root 选择、observe/protect 单热点入口保护、`REFUSED` 拒绝、聚合指标与基础文档；相关 OpenSpec 任务已收口。

**目标**：在 `v1.3.1` 已经限制“单客户端昂贵请求并发”的前提下，继续补上一层“热点 zone / 热点域名维度”的保护，避免流量虽然不是由单个 client IP 独占，但仍集中打爆某个 zone、某组 delegation 或某类随机子域名路径。

**为什么把它提前到下一步**

- per-client 保护回答的是“谁在占用昂贵路径”
- zone 保护回答的是“哪些名字空间正在被集中打爆”
- 这两层组合起来，比单独先做全局保险丝或上游并发更容易解释，也更贴近真实热点流量问题

**下一步要先讨论的问题**

- 保护对象是 `zone`、`delegation cut`、还是更窄的 `fqdn / 随机前缀模式`
- 计数模型是并发、速率，还是“未命中后进入昂贵路径的热点度”
- forwarding zone 和 iterative zone 是否共用一套语义，还是分别保护
- zone 保护与 `v1.3.1` 的 per-client 保护如何组合，避免双重拒绝语义难以解释
- 是否需要区分“热点但健康”和“热点且明显异常”两类信号

**当前 proposal 方向**

- 第一版只保护收包后的昂贵路径入口，不扩展到上游出包保护
- 只解决“多 requester 共同把同一个热点 zone 推入昂贵路径”的场景，不解决多 zone 全局压力
...
- 常态先按 coarse business-root key 做粗粒度占用记录；业务根优先使用 forwarding zone，其次基础后缀（默认值 + 追加），最后回退到三级域名
- 默认基础后缀首版方向：与当前 warmup 默认后缀保持一致；如部署环境需要更多私有 suffix，再通过追加机制补充
- 热点 zone 从短时昂贵占用中动态选择：observe mode 下基于最近几个窗口简单求和，只做单层下钻；若某 child 占据父域主要部分则保护 child，否则保护父域
- 候选热点需连续 `3` 个 `5s` 短窗口成立，主要是为正常业务瞬时启动/初始化流量留缓冲
- observe mode 只在 `avg_expensive_concurrency >= 0.75 * NumCPU()` 且整机 CPU `>= 70%` 时触发；受保护 zone 的新昂贵请求继续用 `REFUSED` 拒绝
- 第一版阈值全部保持为内部常量，不额外暴露用户调参项；默认基础后缀首版列表也已收敛为“与当前 warmup 默认后缀一致”

**不做**

- 不直接扩成完整 WAF / 公网 anti-DDoS 系统
- 不在第一版引入复杂模式识别、自动黑名单或机器学习判定
- 不提前把它和 `v1.3.3` 的上游并发保护混成同一套阈值模型
