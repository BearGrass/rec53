...
### 当前推进主线 — v1.1.5 TUI UX polish 与后续增强优先级收口

**目标**：在 `v1.1.3` 完成 detail 信息层、`v1.1.4` 完成导航闭环后，把 TUI 剩余的 UX 收尾工作和后续增强顺序整理清楚，避免“已完成版本里还挂着未做任务”。
...
- [ ] 明确“轻量趋势提示”、`排序/筛选`、`更完整键位体系` 的版本归属与先后顺序
- [ ] State Machine detail 流程视图：在 `rec53top` 的 detail 中展示完整状态机走向与状态转移路径
...
### v1.1.5 — TUI UX polish 与后续增强优先级收口

**状态**：进行中。当前已完成前两项：`panel drill-down` 和轻量趋势提示。

**目标**：把 `rec53top` 从“信息层和导航都已具备”推进到“版本线清晰、文案和布局更稳、下一批增强顺序明确”的状态，为后续 TUI 增强或 `v1.2.0` 切换提供干净边界。
...
- [ ] 明确 `排序 / 筛选` 何时值得引入，而不是过早增加交互复杂度
- [ ] 明确“更完整的键位体系”应拆分为页签、帮助浮层、target 切换等哪些独立子项
