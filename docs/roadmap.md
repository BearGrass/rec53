...
| v1.1.5 | planned | target | TUI UX polish 与后续增强优先级收口 |
...
### 当前推进主线 — v1.1.5 TUI UX polish、State Machine 收敛与后续增强优先级重排

**目标**：在 `v1.1.3` 完成 detail 信息层、`v1.1.4` 完成导航闭环后，把 TUI 剩余的 UX 收尾工作和后续增强顺序整理清楚，并对 `State Machine` 面板做减法收敛，避免继续在低诊断收益的聚合图形化上投入。

**当前判断**

- `v1.1.1` 到 `v1.1.3` 已经形成连续的 observability/local-ops 交付线
- `rec53top` 已经具备 overview、detail、bounded since-start counters、导航焦点和默认 detail 进入路径
- `v1.1.5` 的前两项已完成：`panel drill-down` 与轻量趋势提示已经落地
- 之前 roadmap 把部分未完成尾项留在 `v1.1.3`，同时没有把 `v1.1.4` 已交付内容及时回写，现已按真实交付拆开
- 对 `State Machine` 的重新评估结论是：聚合“主路径/路径图”在方形 TUI 视图、有循环状态、窗口切分和并发请求混合场景下解释成本高于收益
- 对开发和排障更有价值的能力不是“全局聚合路径图”，而是“指定域名 / 单次查询的真实解析路径”
- 在 TUI 的 UX polish 和后续增强顺序没有收口前，不宜直接把主线切到 `v1.2.0`
...
- [x] 将已交付内容与未完成尾项重新分摊到真实版本，避免“done 版本里仍挂未完成任务”
- [x] 将运行韧性工作顺延到 `v1.2.0`
- [x] 将 DNSSEC 设计与预研顺延到 `v1.2.1`
- [x] 梳理 TUI 后续增强候选的优先级与依赖关系
- [x] 处理文案压缩、布局密度和快捷键提示等 UX polish
- [x] 明确 `panel drill-down` 作为 `v1.1.5` 的第一优先实现项
- [x] 明确“轻量趋势提示”、`排序/筛选`、`更完整键位体系` 的版本归属与先后顺序
- [x] 重新定义 `State Machine` 面板边界：保留状态计数器和终态计数器，不再把聚合路径图当作默认下一优先实现项
- [x] 明确“指定域名真实解析路径”作为后续高价值排障能力候选，并与 `rec53top` 聚合面板分层
...
**状态**：进行中。当前已完成 `panel drill-down`、轻量趋势提示；并已完成对 `State Machine` 聚合路径图价值的重新评估。

**目标**：把 `rec53top` 从“信息层和导航都已具备”推进到“版本线清晰、文案和布局更稳、下一批增强顺序明确”的状态；同时把 `State Machine` 从难以解释的聚合路径图收敛回易读的状态/终态信号，为后续真正高价值的单域名路径追踪留出空间。

**任务**

- [x] 梳理 TUI 后续增强候选的优先级与依赖关系
- [ ] 处理文案压缩、布局密度和快捷键提示等 UX polish
...
- 不再把 `State Machine detail` 聚合真实路径图作为下一优先实现项；它在方形 TUI 视图、有循环状态、并发请求混合和窗口切分场景下解释成本过高
- `State Machine` 的近期方向应改为“少解释、多计数”：优先保留状态计数器、终态出口计数器，以及必要的失败摘要
- 更高价值的后续增强应转向“指定域名真实解析路径”；这类能力更适合面向调试/排障，而不是继续堆叠在聚合看板里
- UX polish 继续留在 `v1.1.5`，因为它不依赖新增指标，主要是对已稳定的信息层、布局密度和 footer/help 提示做收口；当前已完成 footer/help 键位分组与 detail 标题压缩
- “更完整的键位体系”先拆成 `帮助浮层/键位分组`、`页签/视图切换语义`、`target 切换` 三类；其中前两类仍属 `v1.1.x` 可讨论范围，`target 切换` 会触及单 target 边界，应留到后续版本单独评估
- `排序 / 筛选` 不作为当前下一优先实现项；只有当固定 top-N breakdown 已不足以解释问题时，才值得引入以避免过早增加交互复杂度
- 多 target、更深层 drill-down、历史图继续留在 backlog，不回混到 `v1.1.5`

**State Machine 收敛方向**

- [x] 重新定义 `State Machine` 面板的默认问题：它只回答“最近哪个状态变热、哪些终态出口在增长”
- [x] 保留当前窗口状态计数器与关键终态计数器，必要时补少量失败原因聚合
- [x] 不再把聚合主路径、聚合路径树、聚合路径图作为默认主视图目标
- [x] 更新 `docs/user/rec53top/pages.md`、`docs/user/local-ops-tui.md` 与 roadmap，对 `State Machine` 的产品定位和阅读方式重新说明

**单域名真实解析路径候选**

- [x] 定义最小问题模型：给定一个域名，返回一次真实解析里经过的状态序列和终态
- [x] 评估实现落点：日志增强、调试命令、临时 trace 开关，或面向单次请求的独立调试入口
- [x] 明确这类能力与 `rec53top` 聚合面板分层，不把它硬塞进 overview/detail 看板语义
- [x] 输出单独 change proposal，而不是继续作为 `v1.1.5` 中的 UI 小增强隐式推进

**当前排序**

- [x] Priority 1: `panel drill-down`
- [x] Priority 2: 轻量趋势提示（仅最近 `10-20` 个 scrape 的内存序列）
- [x] Priority 3: UX polish（文案压缩、布局密度、footer/help 收口）
- [x] Priority 4: `State Machine` 面板收敛为状态/终态计数器视图
- [ ] Priority 5: `帮助浮层 / 键位分组`
- [ ] Priority 6: `排序 / 筛选`（仅在 fixed top-N 不足时再引入）
- [ ] Priority 7: 单域名真实解析路径方案设计与 change proposal

**不做**

- [ ] 不在这一版一次性实现所有增强候选
- [ ] 不把 `rec53top` 做成历史监控前端；长期趋势继续由 Prometheus / Grafana 承担
- [ ] 不把 runtime resilience 或 DNSSEC 工作重新混入 `v1.1.x`
- [ ] 不继续把聚合状态机图形化当成 `v1.1.5` 的主要交付物
