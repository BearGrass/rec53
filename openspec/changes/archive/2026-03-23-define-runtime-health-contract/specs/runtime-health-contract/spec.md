...
#### Scenario: Service reaches steady state
- **WHEN** the DNS listeners are bound successfully and warmup is not running
...
- **THEN** rec53 SHALL continue startup in `phase=cold-start` or `phase=warming`
...
- **AND** the runtime state SHALL remain machine-distinguishable from normal steady-state service
