...
- [x] `server/xdp_metrics.go` — 周期读取 BPF per-CPU counters，导出为 Prometheus metrics
- [x] `rec53_xdp_cache_hits_total`、`rec53_xdp_cache_misses_total`、`rec53_xdp_pass_total`、`rec53_xdp_errors_total`

**2. TTL 过期清理**

- [x] 周期 goroutine（固定 100ms）：`unix.ClockGettime(unix.CLOCK_MONOTONIC)` 获取当前时间，遍历 BPF map，删除 `expire_ts` 过期条目
- [x] 不做跨 cache 对账（eBPF 内联 expire_ts 检查已保证不服务 stale entries，map 空间浪费可接受）

**3. 性能验收**

- [x] dnsperf benchmark：XDP enabled（generic/lo）vs disabled
- [x] **验收标准**：cache hit QPS 可测量提升，P99 latency 不回退（cache miss 路径不受影响）
- [x] 结果记录到 `docs/benchmarks.md`

**4. 文档**

- [x] `docs/architecture.md` — 新增 XDP Cache 层描述
- [x] `docs/benchmarks.md` — XDP 性能对比数据
- [x] `README.md` / `README.zh.md` — XDP 配置说明、构建依赖、CAP_BPF 权限要求
