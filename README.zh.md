...
- **Prometheus 指标** — 每次查询和每个 NS 服务器均可观测；XDP 缓存计数器（命中/未命中/放行/错误）导出为 Gauge 指标
- **XDP/eBPF 缓存快速路径** — 缓存命中由内核通过 `XDP_TX` 直接返回（零系统调用、零 Go 运行时开销）；自动 TTL 过期清理；需要 Linux 内核 >= 5.15 及 CAP_BPF 权限
...
| 依赖 | 是否必需 | 说明 |
|---|---|---|
| Go 1.21+ | 始终需要 | 核心构建工具链 |
| `clang` >= 14 | 仅 XDP | 编译 eBPF C 代码；运行时不需要 |
| `linux-headers` | 仅 XDP | BPF helper 定义所需的内核头文件 |
| `CAP_BPF` 或 root | 仅 XDP | 运行时加载 XDP 程序并挂载到网络接口所需 |

> **XDP 提示：** XDP/eBPF 支持要求 Linux 内核 >= 5.15。修改 BPF C 代码后，需执行 `go generate ./server/...` 重新生成 Go 绑定。
...
# XDP/eBPF 缓存快速路径：缓存命中由内核通过 XDP_TX 直接返回。
# 零系统调用、零 Go 运行时开销、零内存拷贝。
# 运行时要求：Linux 内核 >= 5.15、CAP_BPF 或 root 权限、clang >= 14（仅构建时需要）。
# xdp:
#   enabled: false
#   interface: ""   # 例如 eth0、ens33
```
...
| `xdp` 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `enabled` | bool | `false` | 启用 XDP/eBPF DNS 缓存快速路径 |
| `interface` | string | `""` | XDP 程序挂载的网络接口（启用时必填） |
